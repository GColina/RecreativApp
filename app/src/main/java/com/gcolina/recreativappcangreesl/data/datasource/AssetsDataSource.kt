package com.gcolina.recreativappcangreesl.data.datasource

import com.gcolina.recreativappcangreesl.data.model.AssetEntity
...
constructor() {
                                    ...
                                    try {
                                        ...
                                        assetEntity?.map { entity -> entity.toDomain() }
                                            ?: return continuation.resume(
                                                Result.failure(Throwable("COULD NOT PARSE!"))
                                            )
                                    } catch (t: Throwable) {
                                        return continuation.resume(
                                            Result.failure(Throwable("ERROR: $t"))
                                        )
                                    }
                                continuation.resume(Result.success(assets))
                            } ?: continuation.resume(
                                Result.failure(Throwable("DATA IS EMPTY!"))
                            )
                        }
    ...
    suspend fun saveAsset(database: FirebaseDatabase, asset: AssetModel): Result<Unit> {
        // 1. Get all assets
    ...
    suspend fun removeAsset(database: FirebaseDatabase, asset: AssetModel): Result<Unit> {
        // 1. Get all assets
        val allAssetsOP = getAssets(database)
        val allAssets =
            if (allAssetsOP.isSuccess) {
    ...
    suspend fun updateAsset(database: FirebaseDatabase, asset: AssetModel): Result<Unit> {
        // 1. Get all assets
        val allAssetsOP = getAssets(database)
        val allAssets =
            if (allAssetsOP.isSuccess) {
